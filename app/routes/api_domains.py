...
async def generate_nginx_config(request: Request):
    ...
    # 读取配置的 HTTPS 端口
    config_data = _reload_config(request)
    https_port = config_data.get("web", {}).get("https_port", 443)
    https_listen = f"listen {https_port} ssl http2;" if https_port != 443 else "listen 443 ssl http2;"
    https_listen_ipv6 = f"listen [::]:{https_port} ssl http2;" if https_port != 443 else "listen [::]:443 ssl http2;"
    redirect_url = f"https://$server_name:{https_port}$request_uri" if https_port != 443 else "https://$server_name$request_uri"
...
async def get_web_config(request: Request):
    """获取 Web 配置（如 HTTPS 端口）"""
    require_auth(request, _get_config(request))
    config = _reload_config(request)
    web_cfg = config.get("web", {})
    return {
        "https_port": web_cfg.get("https_port", 443),
        "web_port": web_cfg.get("port", 5080),
    }


@router.put("/config/https-port")
async def set_https_port(request: Request):
    """设置 HTTPS 端口"""
    require_auth(request, _get_config(request))
    body = await request.json()
    port = body.get("https_port")
